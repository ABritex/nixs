...
function TimelineCard({ entry }: { entry: { year: string; title: string; desc: string; icon: string; tags: string[] } }) {
    return (
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-2">
                        {entry.tags.map((t) => (
                            <span key={t} className="text-[9px] font-mono px-2 py-0.5 bg-muted/30 border border-border/40 rounded text-muted-foreground/60">
                                {t}
                            </span>
                        ))}
                    </div>
...
export default function TimelineSection() {
    ...
    return (
        <section ref={ref} className="relative min-h-screen flex items-start justify-center bg-background px-6 py-24 overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-3xl mx-auto space-y-10">
                ...
                <Tabs defaultValue="work" className="w-full">
                    <TabsList className="scroll-reveal mx-auto w-fit" data-delay="150">
                    ...
                    <TabsContent value="work" className="space-y-4 pt-6">
                        {TIMELINE.work.map((entry, i) => (
                            <div key={i} className="scroll-reveal-left" data-delay={200 + i * 100}>
                    ...
                    <TabsContent value="education" className="space-y-4 pt-6">
                        {TIMELINE.education.map((entry, i) => (
                            <div key={i} className="scroll-reveal-right" data-delay={200 + i * 100}>
                                <TimelineCard entry={entry} />
                            </div>
                        ))}
                    </TabsContent>
